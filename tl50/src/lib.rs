...
//
// Cloneable so that it can be called from multiple places, if needed
...
impl TL50ActorHandle {
    ...
    pub async fn adv_seg_mode(&mut self) {
    ...
    pub async fn steady(&mut self, color: Color, intensity: Intensity) {
        let cmd = AdvSegInd {
            color1: color,
            intensity1: intensity,
            animation: Animation::Steady,
